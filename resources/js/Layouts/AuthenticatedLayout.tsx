    ...
    const getNavItems = (role: string) => {
        ...
        if (role === 'admin') {
            return [
                ...
                { name: 'Book Categories', href: '/admin/book-categories', icon: 'M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10' },
            ...
            return [
                ...
                { name: 'Messages', href: '/author/messages', icon: 'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z' },
        ...
        return [
            ...
            { name: 'Become Author', href: '/become-author', icon: 'M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z' },
        ];
    };
            ...
            <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-slate-900 text-white transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                <div className="flex items-center justify-center h-20 border-b border-white/5">
                    <Link href="/" className="flex items-center gap-3 group">
                        <div className="w-10 h-10 shadow-lg group-hover:scale-110 transition-transform">
                            <ApplicationLogo className="w-full h-full rounded-xl" />
                        </div>
                        <span className="text-xl font-black tracking-[0.2em] text-white uppercase">ELF</span>
                    </Link>
                </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
                ...
                <header className="bg-white/80 backdrop-blur-md border-b border-gray-200 h-20 flex items-center justify-between px-4 sm:px-6 lg:px-8 z-40 sticky top-0">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        {/* Notifications */}
                        <Link
                            href={user.role === 'admin' ? '/admin/messages' : (user.role === 'staff' ? '/staff/messages' : (user.role === 'author' ? '/author/messages' : '/messages'))}
                            className="relative p-2 text-gray-400 hover:text-primary-600 transition-colors rounded-full hover:bg-gray-100"
                        >
                            {auth.unreadCount > 0 && (
